...
export default function HealthPage() {
  ...
  return (
    <Box sx={{ minHeight: '100vh', backgroundColor: '#0A1414', color: '#F5F5F4', p: { xs: 2, md: 6 } }}>
      <Box sx={{ maxWidth: 1440, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 4 }}>
        
        {/* ==================== HEADER STRIP ==================== */}
        <Box component="header" sx={{ borderBottom: '1px solid rgba(104,195,163,0.2)', pb: 4 }}>
          <Typography variant="h4" sx={{ fontWeight: 800, color: '#68C3A3', letterSpacing: '-0.03em' }}>
            Health & Wellness
          </Typography>
        ...
        <Grid container spacing={4}>
          ...
          <Grid item xs={12} lg={7} sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            ...
            <Paper elevation={0} sx={{ p: 4, backgroundColor: 'rgba(17,28,28,0.6)', border: '1px solid rgba(63,63,70,0.2)', borderRadius: 4 }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                <Box>
                  <Typography variant="subtitle1" sx={{ fontWeight: 700, color: '#D6D3D1', display: 'flex', alignItems: 'center', gap: 1 }}>
                    <span style={{ marginRight: '8px' }}>💧</span> Water Hydration Log
                  </Typography>
            ...
            <Paper elevation={0} sx={{ p: 4, backgroundColor: 'rgba(17,28,28,0.6)', border: '1px solid rgba(63,63,70,0.2)', borderRadius: 4, display: 'flex', flexDirection: 'column', gap: 3 }}>
              <Typography variant="subtitle1" sx={{ fontWeight: 700, color: '#D6D3D1', display: 'flex', alignItems: 'center', gap: 1 }}>
                <span style={{ marginRight: '8px' }}>🍱</span> Daily Meal Tracking
              </Typography>
              ...
              <Box component="form" onSubmit={handleAddMeal} sx={{ display: 'flex', flexDirection: 'column', gap: 2, backgroundColor: '#0A1212', p: 2.5, borderRadius: 3, border: '1px solid rgba(63,63,70,0.2)' }}>
                <TextField 
                  fullWidth 
                  placeholder="What did you eat today? Are you tracking muscle gain or fat loss?" 
                  value={newMealName}
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                {meals.map((meal) => (
                  <Box key={meal.id} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', p: 2, borderRadius: 3, backgroundColor: 'rgba(21,36,36,0.4)', border: '1px solid rgba(63,63,70,0.2)', width: '100%', gap: 2 }}>
                    <Box sx={{ flexGrow: 1 }}>
          ...
          <Grid item xs={12} lg={5}>
            <Paper elevation={0} sx={{ p: 4, backgroundColor: 'rgba(17,28,28,0.6)', border: '1px solid rgba(63,63,70,0.2)', borderRadius: 4, display: 'flex', flexDirection: 'column', gap: 4 }}>
              <Box>
                <Typography variant="subtitle1" sx={{ fontWeight: 700, color: '#D6D3D1', display: 'flex', alignItems: 'center', gap: 1 }}>
                  <span style={{ marginRight: '8px' }}>🌸</span> Hormonal & Cycle Rhythm
                </Typography>
                <Typography variant="caption" sx={{ color: '#78716C' }}>Keep a clear record of physical symptom fluctuations.</Typography>
              </Box>

              {/* Cycle Tracker Interface */}
              <Box sx={{ textAlign: 'center', backgroundColor: '#122222', p: 3, borderRadius: 4, border: '1px solid rgba(244,114,182,0.2)' }}>
                ...
                <Box sx={{ mt: 3, display: 'flex', justifyContent: 'center', gap: 1, alignItems: 'center' }}>
                  <Button size="small" onClick={() => setCycleDay(Math.max(1, cycleDay - 1))} sx={{ minWidth: 32, color: '#F472B6', fontSize: '1.2rem' }}>-</Button>
                  ...
                  <Button size="small" onClick={() => setCycleDay(Math.min(28, cycleDay + 1))} sx={{ minWidth: 32, color: '#F472B6', fontSize: '1.2rem' }}>+</Button>
                </Box>
              </Box>
